...
redis_register_script = os.path.abspath(os.path.join(os.path.dirname(__file__), u'../', u'scripts/redis/clerk.py'))


class RedisRegisterContainerAnsiblePostTaskConfigFactory(AnsiblePostTaskConfigFactory):
    template = {
        u'name': u'Register {name} container in redis',
        u'script': redis_register_script + u' --ports="{ports_mappings}" --container_id={container_id} --container_name={container_name}'
    }
